...
async def sample_model(
    ...
    prompt = template.format(prompt=prefill)
    headers = {
...
async def sample_single(
    ...
    try:
        completion, prompt = await sample_model(
            client, model, prefill, temperature, max_tokens, persona, provider
        )
...
async def sample_model_all(
    ...
    tasks = [
        sample_single(
            client, model, prefill, temperature, max_tokens, persona, provider, i
        )
...
async def run_sampling(config: dict) -> None:
    ...
    async with httpx.AsyncClient() as client:
        tasks = []
        for model in models:
            for persona in ["user", "assistant", "standard_no_thinking"]:
                tasks.append(
                    sample_model_all(
                        client,
                        model,
...
def main(config_path: str):
    ...
    asyncio.run(run_sampling(config))
